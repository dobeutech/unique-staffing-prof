/*
  ...
  ## 1. Remove Unused Indexes
    All indexes that are not being utilized will be dropped to:
  ...
  ## 2. Fix Multiple Permissive Policies
    ...
    - Keep single permissive policy per role/action combination
    - Remove redundant authenticated user policies where anon already covers access
...
-- Remove the redundant authenticated policy since "Anyone" covers it
DROP POLICY IF EXISTS "Authenticated users can view all business info" ON business_info;

-- Recreate with proper separation: anon sees active, authenticated sees all
...
-- ============================================================================
-- 4. CREATE ONLY NECESSARY INDEXES
-- ============================================================================

-- Create indexes only for frequently queried columns that need them
...
-- Email verification log - token lookup for verification
...
-- Service areas - commonly filtered by state
CREATE INDEX IF NOT EXISTS idx_service_areas_state_active ON service_areas(state, active)
WHERE active = true;

-- Local testimonials - featured ones for homepage
CREATE INDEX IF NOT EXISTS idx_testimonials_featured_active ON local_testimonials(featured, active)
...
BEGIN
  -- Check business_info has exactly 2 SELECT policies
  ...
  IF policy_count != 2 THEN
    RAISE NOTICE 'Warning: business_info should have exactly 2 SELECT policies, found %', policy_count;
  END IF;

  -- Check local_content has exactly 2 SELECT policies
  SELECT COUNT(*) INTO policy_count
  FROM pg_policies
  WHERE tablename = 'local_content'
    AND cmd = 'SELECT';

  IF policy_count != 2 THEN
    RAISE NOTICE 'Warning: local_content should have exactly 2 SELECT policies, found %', policy_count;
  END IF;

  -- Check local_testimonials has exactly 2 SELECT policies
  SELECT COUNT(*) INTO policy_count
  FROM pg_policies
  WHERE tablename = 'local_testimonials'
    AND cmd = 'SELECT';

  IF policy_count != 2 THEN
    RAISE NOTICE 'Warning: local_testimonials should have exactly 2 SELECT policies, found %', policy_count;
  END IF;

  -- Check service_areas has exactly 2 SELECT policies
  SELECT COUNT(*) INTO policy_count
  FROM pg_policies
  ...
  IF policy_count != 2 THEN
    RAISE NOTICE 'Warning: service_areas should have exactly 2 SELECT policies, found %', policy_count;
  END IF;

  RAISE NOTICE 'Security fixes verification completed';
END $$;
